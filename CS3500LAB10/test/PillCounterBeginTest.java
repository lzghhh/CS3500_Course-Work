...
public class PillCounterBeginTest {
  ...
  @Test
  public void usageDecorator() {
    PillCounter counter = new PillDecorator(new LoggingPillCounter());
    boolean result = conveyerBelt(counter);
    assertTrue(result);
  }
  ...
  public void usageCounter() {
    PillMonitor counter = new PillMonitor(new LoggingPillCounter());
    boolean result = conveyerBelt(counter);
    assertEquals(1600, counter.addedCounts().size());
  }
